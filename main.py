...
active_users = set()  # ترکیب seen_users + active_users
banned_users = set()
...
FIRST_LETTERS = ["آ", "ب", "پ", "ت", "ج", "چ", "ح", "خ", "د", "ر", "ز", "س", "ش", "ص", "ط", "ع", "ف", "ق", "ک", "گ", "ل", "م", "ن", "و", "ه", "ی"]
...
async def check_channel_membership(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """بررسی عضویت در کانال (بدون کش برای تشخیص لحظه‌ای)"""
...
def generate_trivia_question():
    """تولید سوال اطلاعات عمومی با هوش مصنوعی"""
    prompt = """
...
async def stats_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    stats_msg = (
        ...
        f"👥 **تعداد گروه‌ها:** {len(active_users)} گروه\n"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_maintenance_mode and user.username != OWNER_USERNAME:
        await update.message.reply_text("اندروید به دلیل تعمیرات، موقتا غیر فعال است...")
        return

    if user.id in banned_users or chat_id in banned_users:
        await update.message.reply_text("مشکل ارتباطی در سیستم، لطفا بعدا دوباره امتحان کنید...")
        return
    ...
    if chat_type in ["group", "supergroup"]:
        ...
        # گزارش کاربر جدید (فقط برای چت خصوصی)
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # === بازی ۲۰ سوالی ===
    if data == "start_20q":
        await query.edit_message_text("🎲 در حال انتخاب یک کلمه مخفی... ⏳")
        try:
            res_word = co.chat(
                message="یک کلمه ملموس، عامیانه و قابل حدس برای بازی ۲۰ سوالی به زبان فارسی انتخاب کن. فقط خود کلمه را بنویس.",
                model="command-r-08-2024"
            )
            chosen_word = res_word.text.strip().replace("«", "").replace("»", "").replace('"', '')
        except:
            chosen_word = random.choice(["یخچال", "تلسکوپ", "دلفین", "کشتی", "اکسیژن", "کوهستان", "آتشفشان", "موبایل"])

        active_games_20q[chat_id] = {"secret_word": chosen_word, "questions_left": 20, "active": True}
        keyboard = [[InlineKeyboardButton("🏁 انصراف و پایان بازی", callback_data="cancel_game")]]
        await query.edit_message_text(
    ...
    # === بازی اطلاعات عمومی ===
        ...
        await query.edit_message_text("⏳ در حال طراحی سوال...")
        ...
        correct = trivia_games[chat_id]["correct_index"]
        correct_text = trivia_games[chat_id]["options"][correct]

        if user_choice == correct:
            res_text = f"🎉 **آفرین {user.first_name}!** پاسخ شما کاملاً درست بود! 🏆\nگزینه صحیح: **{correct_text}**"
        else:
            res_text = f"❌ **اشتباه بود {user.first_name} عزیز!**\nپاسخ صحیح گزینه **«{correct_text}»** بود. 😉"
    ...
    # === بازی جاسوس ===
        ...
        if query.message.chat.type not in ["group", "supergroup"]:
            await query.edit_message_text("⚠️ **بازی جاسوس فقط مخصوص گروه‌هاست!**")
            return
        ...
        await query.edit_message_text(
            f"🕵️‍♂️ **اتاق بازی جاسوس**\n\n👤 **اعضای حاضر ({len(players)} نفر):**\n{players_list}\n\nهر وقت همه‌ جمع شدید، دکمه «🚀 شروع بازی!» رو بزنید.",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown"
        )
        ...
        await query.edit_message_text(start_msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    ...
    # === بازی داستان گروهی ===
        ...
        story_games[chat_id] = {"active": True, "sentences": [f"🎬 {starter}"]}
        ...
        if chat_id not in story_games or not story_games[chat_id]["active"]:
            ...
            return
        full_story = " ".join(story_games[chat_id]["sentences"])
        del story_games[chat_id]
        await query.edit_message_text(
            f"📖 **داستان کامل گروهی شما:** 🏆\n\n{full_story}\n\nدست همگی درد نکنه! 👏😍",
            parse_mode="Markdown"
        )
    ...
    # === بازی من کی‌ام؟ ===
        ...
        if query.message.chat.type not in ["group", "supergroup"]:
            await query.edit_message_text("⚠️ **بازی «من کی‌ام؟» مخصوص گروه‌هاست!**")
            return
        ...
        await query.edit_message_text(start_text, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    ...
    # === بازی اسم بازی ===
        ...
        context.job_queue.run_once(name_game_timeout, TIMER_SECONDS, chat_id=chat_id, name=f"timer_{chat_id}")

        keyboard = [[InlineKeyboardButton("🏁 انصراف و پایان بازی", callback_data="cancel_game")]]
        await query.edit_message_text(
            f"🔤 **بازی «اسم بازی» (زنجیره کلمات) شروع شد!**\n\n"
            f"📜 **قواعد بازی:**\n۱. کلمه‌ای بگو که با **حرف آخر** کلمه نفر قبلی شروع بشه.\n"
            f"۲. کلمات تکراری قبول نیستند!\n۳. برای هر کلمه فقط **{TIMER_SECONDS} ثانیه** فرصت داری!\n\n"
...
async def track_chats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if old_status in ["left", "kicked"] and new_status in ["member", "administrator"]:
        admin_list = ""
        try:
            for admin in await context.bot.get_chat_administrators(chat.id):
                u = admin.user
                admin_list += f"• {u.full_name} | @{u.username if u.username else 'ندارد'} | `{u.id}`\n"
    ...
    if chat_type in ["group", "supergroup"]:
        ...
        if chat_id in name_games and name_games[chat_id]["active"]:
            ...
            if len(word.split()) == 1 and not word.startswith("/"):
                first_char = word[0].replace("ك", "ک").replace("ي", "ی")
        ...
        if chat_id in story_games and story_games[chat_id]["active"]:
            ...
            await update.message.reply_text(
                f"✅ کلمات {user_name} به داستان اضافه شد!\n\n👇 نفر بعدی ادامه بده:",
                reply_markup=InlineKeyboardMarkup(keyboard)
            )
            return
        ...
        # بررسی عضویت برای منشن/ریپلای
        is_replied_to_bot = update.message.reply_to_message and update.message.reply_to_message.from_user.id == context.bot.id
        is_mentioned = f"@{bot_username}" in user_text

        # بازی ۲۰ سوالی در گروه
        ...
        # اگر کاربر ربات رو صدا زده، عضویت رو چک کن
